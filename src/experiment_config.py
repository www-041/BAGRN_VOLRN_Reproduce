...
def validate_config(config: ExperimentConfig, skip_file_check: bool = False) -> List[str]:
    ...
    if hasattr(config, 'registration_params'):
        ...
        klt_validation_enabled = (
            backend == "klt_tps"
            or any(key in rp for key in _KLT_TPS_PARAM_KEYS)
        )
        ...
        for key, minimum in (klt_integer_minimums.items() if klt_validation_enabled else []):
            value = rp.get(key)
        ...
        if klt_validation_enabled and (isinstance(window, bool) or not isinstance(window, (int, np.integer))
        ...
        if klt_validation_enabled and (isinstance(pyramid, bool) or not isinstance(pyramid, (int, np.integer))
                or not 0 <= int(pyramid) <= 8):
            errors.append("registration_params.klt_tps_pyramid_level 必须是 0 到 8 的整数")
        for key in (("klt_tps_min_corner_distance", "klt_tps_fb_threshold", "klt_tps_max_shift") if klt_validation_enabled else ()):
            try:
        ...
        try:
            ...
            smoothing = float("nan")
        if klt_validation_enabled and (not np.isfinite(smoothing) or smoothing < 0):
