"""Unified five-scene SIFT/LoFTR + BAGRN + VOLRN mosaic CLI."""
...
def build_parser(default_matcher: str | None = None) -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Five-scene matcher-selectable registration + BAGRN + VOLRN mosaic"
        )
    )
    ...
    parser.add_argument(
        ...
        required=default_matcher is None,
        help="Tie-point matcher. Downstream geometry/radiometry is shared.",
    )
    parser.add_argument("--registration-band", default="B14",
                        help="Band for geometric registration (default: B14)")
    parser.add_argument("--bands", default="B14,B8,B5",
                        help="Comma-separated radiometric bands (default: B14,B8,B5)")
    ...
    parser.add_argument("--ransac-threshold", type=float, default=2.0,
                        help="Shared affine RANSAC threshold in pixels (default: 2.0)")
    parser.add_argument("--random-seed", type=int, default=0,
                        help="Shared RANSAC random seed (default: 0)")

    parser.add_argument(
        "--radiometric-control-index",
        type=int,
        required=True,
        help=(
            "Scene index used as BAGRN radiometric control image "
            "(independent from geometry reference)"
        ),
    )
...
def main(default_matcher: str | None = None) -> None:
    args = build_parser(default_matcher=default_matcher).parse_args()
    ...
    result = run_five_scene_mosaic(
        ...
        radiometric_control_idx=args.radiometric_control_index,
